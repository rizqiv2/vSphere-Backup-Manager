...
def load_jobs_db(is_startup=False):
    global jobs
    init_db()
    migrate_old_json_db()
    try:
        ...
        # Clean up any jobs left in running/queued state across restart (only at startup)
        if is_startup:
            updated_jobs = []
            with jobs_db_lock:
